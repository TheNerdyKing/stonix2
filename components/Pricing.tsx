...
export default function Pricing() {
  ...
  useGSAP(() => {
    ...
    // Pricing cards: slide in from left → right, staggered
        ...
        <div ref={cardsRef} className="grid md:grid-cols-3 gap-6 items-start">
          {dict.pricing.plans.map((p: any, i: number) => {
            ...
            return (
              <div
                key={p.name}
                className={`plan-card rounded-[2rem] p-8 flex flex-col relative overflow-hidden h-full ${isPopular ? "popular-border-orbit" : ""}`}
                style={{
                  background: isPopular ? "#0E1118" : "rgba(14,17,24,0.5)",
                  border: isPopular ? "1px solid rgba(249,115,22,0.15)" : "1px solid rgba(255,255,255,0.06)",
                  boxShadow: isPopular ? "0 0 80px rgba(249,115,22,0.1)" : "none",
                  isolation: "isolate",
                }}
              >
                {/* Top edge highlight (popular only) */}
                {isPopular && (
                  <div className="absolute top-0 inset-x-0 h-px z-10" style={{ background: "linear-gradient(90deg, transparent, #F97316, transparent)" }} />
                )}
                {/* Subtle bottom glow */}
                {isPopular && (
                  <div className="absolute bottom-0 inset-x-0 h-40 pointer-events-none z-0" style={{ background: "linear-gradient(to top, rgba(249,115,22,0.06), transparent)" }} />
                )}

                {isPopular && (
                  <div className={`flex items-center mb-4 z-10 relative ${dir === "rtl" ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`${dir === "rtl" ? "text-right" : "text-left"} mb-6 relative z-10`}>
                ...
                <div className={`flex items-baseline gap-1 mb-8 relative z-10 ${dir === "rtl" ? "justify-end flex-row-reverse" : "justify-start"}`}>
                ...
                <a href="#contact" className="w-full relative z-10">
                  {isPopular ? (
                    <button className="wave-btn w-full py-3.5 rounded-xl font-bold text-sm text-black">
                      {p.cta}
                    </button>
                  ) : (
                    <button
